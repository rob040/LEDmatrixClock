...
* [Examples](#examples)
  ...
  * [ 5. checkWaitingMRD](examples/checkWaitingMRD)
...
### Why do we need this [ESP_MultiResetDetector library](https://github.com/rob040/LEDmatrixClock/lib/ESP_MultiResetDetector)

#### Features

[**ESP_MultiResetDetector**](https://github.com/rob040/LEDmatrixClock/lib/ESP_MultiResetDetector) version 2.0 is a library for the **ESP8266 and ESP32** boards to detect a **configurable multi reset**, within configurable timeout (default 10s) seconds, so that an alternative start-up mode can be used. Example use cases are to allow re-configuration of a device's WiFi / MQTT / Blynk credentials or to count the number of resets within a pre-determined timed.

https://github.com/rob040/LEDmatrixClock/lib/ESP_MultiResetDetector (version 2.0+)
This library is version 2 of [khoih ESP_MultiResetDetector](https://github.com/khoih-prog/ESP_MultiResetDetector) to simplify the configuration and make the application of the MultiResetDetector
safer by not using non-volatile storage like SPIFFS or LittleFS or flash emulated EEPROM, but using volatile storage (RAM).
Finally, reset events shall be detected when target remains powered but receives (multiple) resets. It is not there to remember reset events that happened a week ago or such, hence non-volatile storage is not needed, just some memory locations that survive reset and application restart. But more important, it is never a good idea to write to NVS filesystems when it is likely that resets are going to happen; it creates data corruption, filesystem corruption and possibly flash memory cell defects due to incomplete write cycles.

The khoih library (version 1.x) was based on  [`Stephen Denne's DoubleResetDetector`](https://github.com/datacute/DoubleResetDetector) but did add support to ESP8266 and ESP32 using EEPROM, SPIFFS and LittleFS besides original RTC. Those addition are now questioned as being destructive to the flash filesystem, the configuration options near endless and far from simple to use.

Khioh also created and maintained a [`DoubleResetDetector`](https://github.com/khoih-prog/ESP_DoubleResetDetector), but having a MultiResetDetector which can also detect 2 in succession, the DoubleResetDetector appears superfluous, yet it soldered on till many of Khioh's Arduino libraries were frozen in 'Archived' state.

This library can be used to detect a **multi reset within a predetermined time to force the program to enter a special operation** such as Config Portal, Clear Default Data, etc.
...
This [**ESP_MultiResetDetector** library](https://github.com/khoih-prog/ESP_MultiResetDetector) currently supports these following boards:
 ...
 1. **ESP32, ESP32_C3, ESP32_S2 and ESP32_S3 boards**.
 2. **ESP8266 boards**

There are no filesystem dependencies anymore and very little configuration options, which makes the different examples boringly identical and simple (they weren't in version 1.x !).
---
---
...
```cpp
// Optionally enable debug serial output to read what is going on
...
// RTC Memory Address for the MultiResetDetector to use (only appliccable to ESP8266)
#define MRD_ADDRESS             0

#include <ESP_MultiResetDetector.h>      // https://github.com/rob040/LEDmatrixClock/lib/ESP_MultiResetDetector 2.0+
...
#### 3. [ESP_WiFiManager_Lite Library](https://github.com/khoih-prog/ESP_WiFiManager_Lite)

** TO BE CHANGED **

* [ 1. Async_ConfigOnDoubleReset](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ConfigOnDoubleReset)
* [ 2. Async_ConfigOnDRD_FS_MQTT_Ptr](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ConfigOnDRD_FS_MQTT_Ptr)
* [ 3. Async_ESP32_FSWebServer_DRD](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ESP32_FSWebServer_DRD)
* [ 4. Async_ESP_FSWebServer_DRD](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ESP_FSWebServer_DRD)
* [ 5. Async_ConfigOnDRD_ESP32_minimal](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ConfigOnDRD_ESP32_minimal)
* [ 6. Async_ConfigOnDRD_ESP8266_minimal](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ConfigOnDRD_ESP8266_minimal)
* [ 7. Async_ConfigOnDRD_FS_MQTT_Ptr_Complex](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ConfigOnDRD_FS_MQTT_Ptr_Complex)
* [ 8. Async_ConfigOnDRD_FS_MQTT_Ptr_Medium](https://github.com/khoih-prog/ESP_WiFiManager_Lite/tree/master/examples/Async_ConfigOnDRD_FS_MQTT_Ptr_Medium)
...
** TO BE CHANGED to library version 2.0+ **
